...
export async function POST(
  ...
  const { session, error } = await requireAdminAuth('providers.manage')
  ...
  const { error: dbErr } = await adminQuery(`ai_providers?id=eq.${providerId}`, {
    method: 'PATCH',
  ...
  return NextResponse.json({ ok: true, action, providerId })
}
